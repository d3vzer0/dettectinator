...
class DettectBase(object):
    ...
    def __init__(
        self, filename: str, domain: str = None, local_stix_path: str = None
    ) -> None:
        ...
        if filename is not None:
            ...
            self.domain = (
                "enterprise-attack"
                if "domain" not in self._yaml_content.keys()
                else self._yaml_content["domain"]
            )
        else:
            self.domain = (
                "mobile-attack"
    ...
    def _init_yaml(self) -> None:
        ...
        """
        self._yaml = YAML()
        self._yaml.Representer.ignore_aliases = (
            lambda *args: True
    ...
    def start_clean_file(self):
        ...
        if isinstance(self, DettectDataSourcesAdministration):
            ...
            if self.domain == "mobile-attack":
                raise Exception(
                    f"{self.__class__.__name__}: Data sources for ATT&CK Mobile not yet implemented by MITRE."
                )
    ...
    def save_yaml_file(self, filename=None) -> None:
        ...
        with open(filename if filename else self.filename, "w") as yaml_file:
            ...
            for line in new_lines:
                ...
                if "date" in line:
                    line = line.replace("T00:00:00Z", "T00:00:00.000Z").replace(
                        " 00:00:00", "T00:00:00.000Z"
                    )
    ...
    @staticmethod
    def _add_entry_to_list_in_dictionary(
        dictionary: dict, key_dict: str, key_list: str, entry: object
    ) -> None:
        """
    ...
    def _load_platform_in_correct_capitalisation(self) -> None:
        ...
        selected_platforms = (
            ...
            if self.domain == "ics-attack"
            else PLATFORMS_MOBILE
        )
    ...
    def _initialize_attack_client(self) -> None:
        ...
        try:
            if self._local_stix_path is not None:
                if (
                    self._local_stix_path is not None
                    and os.path.isdir(
                        os.path.join(self._local_stix_path, "enterprise-attack")
                    )
                    and os.path.isdir(os.path.join(self._local_stix_path, "ics-attack"))
                    and os.path.isdir(
                        os.path.join(self._local_stix_path, "mobile-attack")
                    )
                ):
                    self.mitre = attack_client(local_path=self._local_stix_path)
                else:
                    raise Exception(
                        f"{self.__class__.__name__}: Not a valid local STIX path: {self._local_stix_path}"
                    )
            else:
            ...
            if hasattr(e, "request"):
                raise Exception(
                    f"{self.__class__.__name__}: Cannot connect to MITRE's CTI TAXII server: {str(e.request.url)}"
                ) from e
            else:
                raise Exception(
                    f"{self.__class__.__name__}: Cannot connect to MITRE's CTI TAXII server."
                ) from e
    ...
    def _load_attack_techniques(self) -> None:
        ...
        """
        if self.domain == "enterprise-attack":
            self.attack_techniques = self._convert_stix_techniques_to_dict(
                self.mitre.get_enterprise_techniques()
            )
        elif self.domain == "ics-attack":
            self.attack_techniques = self._convert_stix_techniques_to_dict(
                self.mitre.get_ics_techniques()
            )
        else:
            self.attack_techniques = self._convert_stix_techniques_to_dict(
                self.mitre.get_mobile_techniques()
            )
...
class DettectTechniquesAdministration(DettectBase):
    ...
    def __init__(
        self, filename: str = None, domain: str = None, local_stix_path: str = None
    ) -> None:
        super(DettectTechniquesAdministration, self).__init__(
            filename, domain, local_stix_path
        )
    ...
    def update_detections(
        ...
        check_unused_detections: bool = False,
        clean_unused_detections: bool = False,
        location_prefix_unused_detections: str = "",
        check_unused_applicable_to: bool = False,
        clean_unused_applicable_to: bool = False,
    ) -> list:
        """
        ...
        w, r = self._delete_rules(
            detection_rules,
            check_unused_detections,
            clean_unused_detections,
            location_prefix_unused_detections,
            check_unused_applicable_to,
            clean_unused_applicable_to,
            date_today,
        )
        warnings += w
        results += r
    ...
    def _add_rules(self, detection_rules: dict, date_today: datetime) -> list:
        ...
        for rule_name, rule_data in detection_rules.items():
            ...
            for technique_id in rule_data["techniques"]:
                ...
                location = (
                    rule_name
                    if rule_data["location_prefix"] == ""
                    else rule_data["location_prefix"] + ": " + rule_name
                )
                ...
                if yaml_technique is not None:
                    ...
                    applicable_to_list = [
                        d["applicable_to"] for d in yaml_technique["detection"]
                    ]
                    if rule_data["applicable_to"] in applicable_to_list:
                                ...
                                if not rule_exist:
                                    ...
                                    for logbook_entry in d["score_logbook"]:
                                        if logbook_entry["date"] == date_today:
                                            logbook_entry[
                                                "comment"
                                            ] += f". Detection rule added: {rule_name}"
                                            today_found = True
                                            break
                        ...
                        new_detection["score_logbook"][0][
                            "comment"
                        ] = f"Auto added by Dettectinator. TODO: Check score. applicable_to with detection rule added: {rule_name}"
                        yaml_technique["detection"].append(new_detection)
                        results.append(
                            f'Check score for technique {technique_id}. applicable_to is new: {str(rule_data["applicable_to"])}.'
                        )
                else:
                    ...
                    new_detection["score_logbook"][0][
                        "comment"
                    ] = f"Auto added by Dettectinator. TODO: Check score. Technique with detection rule added: {rule_name}"
                    ...
                    results.append(
                        f"Check score for technique {technique_id}. Technique is new."
                    )
    ...
    def _delete_rules(
        self,
        detection_rules: dict,
        check_unused_detections: bool,
        clean_unused_detections: bool,
        location_prefix_unused_detections: str,
        check_unused_applicable_to: bool,
        clean_unused_applicable_to: bool,
        date_today: datetime,
    ) -> list:
        """
        ...
        for rule_name, rule_data in detection_rules.items():
            if str(rule_data["applicable_to"]) not in applicable_to_rules.keys():
                applicable_to_rules[
                    str(rule_data["applicable_to"])
                ] = (
                    {}
                )  # cast key to string because a list cannot be a dictionary index name
        ...
        for yaml_technique in self._yaml_content["techniques"]:
            ...
            for detection in yaml_technique["detection"]:
                ...
                if str(detection["applicable_to"]) in applicable_to_rules.keys():
                    ...
                    rules_for_app_to = []
                    for rule_name, rule_data in applicable_to_rules[
                        str(detection["applicable_to"])
                    ].items():
                    ...
                    for loc in locations:
                        if (
                            loc.startswith(location_prefix_unused_detections)
                            and loc not in rules_for_app_to
                        ):
                            if check_unused_detections and not clean_unused_detections:
                                warnings.append(
                                    "Rule from YAML not found in rules list: " + loc
                                )
                            elif check_unused_detections and clean_unused_detections:
                                detection["location"].remove(loc)
                                score = (
                                    0
                                    if len(detection["location"]) == 0
                                    else self._get_latest_score(detection)
                                )
                                ...
                                if not today_found:
                                    ...
                                    results.append(
                                        f'Check score for technique {yaml_technique["technique_id"]}. Detection rule(s) removed.'
                                    )
                else:
        ...
        for d in self._yaml_content["techniques"]:
            if "detection" in d:
                ...
                if isinstance(
                    ...
                    self._add_entry_to_list_in_dictionary(
                        my_techniques, d["technique_id"], "detection", d["detection"]
                    )
                elif isinstance(
                    d["detection"], list
                    ...
                    for de in d["detection"]:
                        ...
                        self._add_entry_to_list_in_dictionary(
                            my_techniques, d["technique_id"], "detection", de
                        )
            ...
            if "visibility" in d:
                ...
                if isinstance(
                    d["visibility"], dict
                ):  # There is just one visibility entry
                    d["visibility"] = self._set_yaml_dv_comments(d["visibility"])
                    self._add_entry_to_list_in_dictionary(
                        my_techniques, d["technique_id"], "visibility", d["visibility"]
                    )
                elif isinstance(
                    d["visibility"], list
                ):  # There are multiple visibility entries
                    for de in d["visibility"]:
                        de = self._set_yaml_dv_comments(de)
                        self._add_entry_to_list_in_dictionary(
                            my_techniques, d["technique_id"], "visibility", de
                        )
    ...
    def _get_latest_score_obj(yaml_object: object) -> object:
        ...
        if (
            len(yaml_object["score_logbook"]) > 0
            and "date" in yaml_object["score_logbook"][0]
        ):
            ...
            for score_obj in yaml_object["score_logbook"]:
                ...
                if not newest_score_obj or (
                    score_obj_date and score_obj_date > newest_date
                ):
...
class DettectDataSourcesAdministration(DettectBase):
    ...
    def __init__(
        ...
        super(DettectDataSourcesAdministration, self).__init__(
            filename, domain, local_stix_path
        )
    ...
    def update_data_sources(
        self,
        data_sources: dict,
        check_unused_data_sources: bool = False,
        clean_unused_data_sources: bool = False,
    ) -> list:
        """
    ...
    def _add_data_sources(self, data_sources: dict, date_today: datetime) -> list:
        ...
        for data_source_name, data_source_applicable_to in data_sources.items():
            if data_source_name not in self.data_components:
                results.append(
                    f'Data source "{data_source_name}" does not exist in ATT&CK ({self.domain}). Skipping.'
                )
                continue
            ...
            if yaml_data_source is None:
                ...
                results.append(
                    f"Check data quality scores for data source {data_source_name}. Data source is new."
                )
            ...
            for data_source_data in data_source_applicable_to:
                ...
                for a in data_source_data["applicable_to"]:
                    if a not in self._system_applicable_to_values:
                        self._yaml_content["systems"].append(
                            {"applicable_to": a, "platform": ["all"]}
                        )
                ...
                applicable_to_list = [
                    d["applicable_to"] for d in yaml_data_source["data_source"]
                ]
                if data_source_data["applicable_to"] not in applicable_to_list:
                    ...
                    new_data_source["available_for_data_analytics"] = data_source_data[
                        "available_for_data_analytics"
                    ]
                    default_scores = self._set_data_quality(
                        new_data_source, data_source_data
                    )
                    new_data_source["comment"] = "Auto added by Dettectinator." + (
                        " TODO: Check data quality scores, default values used."
                        if default_scores
                        else ""
                    )
                    ...
                    for data_source in yaml_data_source["data_source"]:
                        if (
                            data_source["applicable_to"]
                            == data_source_data["applicable_to"]
                        ):
                            data_source["products"] = data_source_data["products"]
                            data_source[
                                "available_for_data_analytics"
                            ] = data_source_data["available_for_data_analytics"]
    ...
    def _delete_data_sources(
        self, data_sources: list, clean_unused_data_sources: bool
    ) -> list:
        """
        ...
        for yaml_data_source in self._yaml_content["data_sources"]:
            ...
            if yaml_data_source["data_source_name"] in data_sources.keys():
                ...
                if not clean_unused_data_sources:
                    warnings.append(
                        f'YAML data source "{yaml_data_source["data_source_name"]}" not in given data sources list.'
                    )
                else:
        ...
        for item in to_remove:
            ...
            warnings.append(
                f'YAML data source "{item["data_source_name"]}" not in given data sources list, so removed.'
            )
    ...
    def _set_data_quality(new_data_source_object, data_source_data):
        if (
            "data_quality" in data_source_data.keys()
            and data_source_data["data_quality"] != {}
        ):
            new_data_source_object["data_quality"][
                "device_completeness"
            ] = data_source_data["data_quality"]["device_completeness"]
            new_data_source_object["data_quality"][
                "data_field_completeness"
            ] = data_source_data["data_quality"]["data_field_completeness"]
            new_data_source_object["data_quality"]["timeliness"] = data_source_data[
                "data_quality"
            ]["timeliness"]
            new_data_source_object["data_quality"]["consistency"] = data_source_data[
                "data_quality"
            ]["consistency"]
            new_data_source_object["data_quality"]["retention"] = data_source_data[
                "data_quality"
            ]["retention"]
            return False
        else:
