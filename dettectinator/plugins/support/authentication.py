...
class Azure:
    ...
    def connect_device_flow(app_id: str, tenant_id: str, endpoint: str) -> str:
        ...
        if "user_code" not in flow:
            raise Exception(
                "Azure: Failed to create device flow. Err: %s"
        ...
        if "access_token" in logon_result:
            ...
            print(
                f'Token expiration: {datetime.datetime.fromtimestamp(logon_result["id_token_claims"]["exp"]).isoformat()}'
            )
    ...
    @staticmethod
    def connect_client_secret(
        app_id: str, tenant_id: str, endpoint: str, secret: str
    ) -> str:
        """
        ...
        app = msal.ConfidentialClientApplication(
            app_id, authority=authority, client_credential=secret
        )
