...
class TechniqueAzureAuthBase(TechniqueBase):
    ...
    def __init__(self, parameters: dict) -> None:
        ...
        if "app_id" not in self._parameters:
            raise Exception(
                f'{self.__class__.__name__}: "app_id" parameter is required.'
            )

        if "tenant_id" not in self._parameters:
            raise Exception(
                f'{self.__class__.__name__}: "tenant_id" parameter is required.'
            )
    ...
    def _connect_to_azure(self, endpoint: str) -> str:
        if self._secret:
            return Azure.connect_client_secret(
                self._app_id, self._tenant_id, endpoint, self._secret
            )
        else:
...
class TechniqueSentinelAlertRules(TechniqueAzureAuthBase):
    ...
    def __init__(self, parameters: dict) -> None:
        ...
        if "subscription_id" not in self._parameters:
            raise Exception(
                'DetectionSentinelAlertRules: "subscription_id" parameter is required.'
            )

        if "resource_group" not in self._parameters:
            raise Exception(
                'DetectionSentinelAlertRules: "resource_group" parameter is required.'
            )

        if "workspace" not in self._parameters:
            raise Exception(
                'DetectionSentinelAlertRules: "workspace" parameter is required.'
            )
    ...
    def set_plugin_params(parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--subscription_id",
            help="Azure subscription id for Sentinel",
            required=True,
        )
        parser.add_argument(
            "--resource_group", help="Azure resource group for Sentinel", required=True
        )
        parser.add_argument(
            "--workspace", help="Azure workspace for Sentinel", required=True
        )
    ...
    def _get_sentinel_data(self, access_token: str) -> list:
        ...
        """
        url = (
            f"https://management.azure.com/subscriptions/{self._subscription_id}/resourceGroups/{self._resource_group}/"
        ...
        if response.status_code != requests.codes["ok"]:
            ...
            if response.status_code == requests.codes["too_many_requests"]:
                raise ConnectionRefusedError(
                    "DetectionSentinelAlerts: You have likely hit the API limit. "
                )
