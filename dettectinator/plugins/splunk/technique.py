...
class TechniqueSplunkConfigSearches(TechniqueBase):
    ...
    def __init__(self, parameters: dict) -> None:
        ...
        if "file" not in self._parameters:
            raise Exception(
                'DetectionSplunkConfigSearches: "file" parameter is required.'
            )
    ...
    def set_plugin_params(parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--file",
            help="Path of the savedsearches config file to import",
            required=True,
        )
    ...
    def get_data_from_source(self) -> Iterable:
        ...
        for section in splunk_config.sections():
            if (
                splunk_config[section].name in ignore_list
                or "action.correlationsearch.label" not in splunk_config[section].keys()
                or "action.correlationsearch.annotations"
                not in splunk_config[section].keys()
                or (
                    "disabled" in splunk_config[section].keys()
                    and splunk_config[section]["disabled"] == "1"
                )
            ):
                continue
            ...
            try:
                annotations = json.loads(
                    splunk_config[section]["action.correlationsearch.annotations"]
                )
            except Exception as e:
                print(
                    f"Could not parse mitre_attack entry in action.correlationsearch.annotations ({str(e)}): {splunk_config[section].name}"
                )
            else:
