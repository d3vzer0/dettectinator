...
class DatasourceSplunkSourcetypes(DatasourceOssemBase):
    ...
    def __init__(self, parameters: dict) -> None:
        ...
        if (
            not is_token_auth
            and not "username" in self._parameters
            and not "password" in self._parameters
        ):
            raise Exception(
                f'{self.__class__.__name__}: "No token or username/passsword provided'
            )
    ...
    def set_plugin_params(parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--username", help="Username of Splunk user (if applicable)", required=False
        )
        parser.add_argument(
            "--password", help="Password of Splunk user (if applicable)", required=False
        )
        parser.add_argument(
            "--token", help="Token of Splunk user (if applicable).", required=False
        )
